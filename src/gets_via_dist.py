...
def compare_all_copy(all_distances):
    first_notes = [round((x + y)/2, 2) for x, y in zip(all_distances[0], all_distances[2])]
    rep_notes = [round((x + y)/2, 2) for x, y in zip(all_distances[1], all_distances[3])]
    #print(first_notes)
    #print(rep_notes)
    print_bar(first_notes, rep_notes)
...
#pritn_bp(get_euclidean_results(all_mlkit_points))
#compare_all_copy(compare_copy(all_mlkit_points))
