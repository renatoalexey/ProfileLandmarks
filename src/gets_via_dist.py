...
def print_bar(first_notes, rep_notes):
    ...
    plt.bar(x - width/2, first_notes, width, label='Primeira anotação')
    plt.bar(x + width/2, rep_notes, width, label='Segunda anotação')
...
def compare_all_copy(all_distances_copy):
    #first_notes = [round((x + y)/2, 2) for x, y in zip(all_distances_copy[0], all_distances_copy[2])]
    #rep_notes = [round((x + y)/2, 2) for x, y in zip(all_distances_copy[1], all_distances_copy[3])]
    copy_means = [round((x + y)/2, 2) for x, y in zip(all_distances_copy[0], all_distances_copy[1])]
    #print(first_notes)
    #print(rep_notes)
    teste = get_euclidean_results(all_mlkit_points)
    mlkit_means = [fmean(valores) for valores in zip(*teste.values())]
...
#pritn_boxplot(get_euclidean_results(all_mlkit_points))
